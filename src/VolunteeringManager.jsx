...
export default function VolunteeringManager({ onBack }) {
  ...
  async function handleSubmit(e) {
    ...
    let result;
    if (editSession) {
      result = await supabase.from('volunteer_sessions').update(payload).eq('id', editSession.id);
    } else {
      result = await supabase.from('volunteer_sessions').insert(payload);
    }
    setSaving(false);
  ...
  async function handleDelete(id) {
    if (!window.confirm('Delete this volunteer session and all its attendance records?')) return;
    const supabase = getSupabaseClient();
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-white">Volunteering</h1>
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-800 bg-slate-900 w-full">
          <table className="min-w-full divide-y divide-slate-800">
            ...
            <tbody className="divide-y divide-slate-800">
              {sessions.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-4 py-5 text-sm text-slate-400">No volunteer sessions found.</td>
                </tr>
              ) : (
                sessions.map((s) => (
                  <tr key={s.id}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="w-full max-w-md rounded-xl bg-slate-900 p-6 border border-slate-700">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-1" htmlFor="vol_session_notes">Notes</label>
                <textarea
                  id="vol_session_notes"
                  value={form.notes}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-lg rounded-xl bg-slate-900 p-6 border border-slate-700 shadow-2xl">
            <h2 className="text-xl font-semibold text-white">
              Volunteers &mdash; {activeSession.session_date}
            </h2>
            <p className="mt-1 text-sm text-slate-400">Check the players who volunteered for this session.</p>
            ...
            <div className="mt-3 flex items-center gap-3 text-sm text-slate-400">
              <span>{attendanceSet.size} / {players.length} volunteered</span>
              <button
                type="button"
